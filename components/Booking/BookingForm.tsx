...
function createBookingSchema(tValidation: (key: string) => string) {
  return z.object({
    ...
    mobile: z.string().min(1, tValidation('mobileRequired')).regex(/^[+]?[(]?[0-9]{1,4}[)]?[-\s.]?[(]?[0-9]{1,4}[)]?[-\s.]?[0-9]{1,9}$/, tValidation('mobileInvalid')),
    date: z.string().min(1, tValidation('dateRequired')),
  });
}

export default function BookingForm({ eventSlug, eventTitle }: BookingFormProps) {
